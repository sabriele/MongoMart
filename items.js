...
function ItemDAO(database) {
	// "use strict";
	this.db = database;
	// Does this take the input of database name and then passes it on for use  
	...
	this.getCategories = function (callback) {
		...
		this.db.collection("item").aggregate([
			...
			categories.unshift(all); // Add the 'All' category into categories
	...
	this.getItems = function (category, page, itemsPerPage, callback) {
		...
		if (category === "All") {
			query = [
				{ $sort: { _id: 1 } },
				{ $skip: page * itemsPerPage }, // page is initiated at zero as in mongomart.js:69
				{ $limit: itemsPerPage }
			];
		} else {
			query = [
		...
		this.db.collection("item").aggregate(query).toArray(function (err, pageItems) {
			assert.equal(err, null);
			callback(pageItems);
		});
	};
	...
	this.searchItems = function (query, page, itemsPerPage, callback) {
		...
		this.db.collection("item").aggregate(searchQuery).toArray(function (err, items) {
			assert.equal(err, null);
			callback(items);
		});
	};
